...
// PATH: my-website/src/app/blog/[slug]/page.js
// CHANGES:
//   1. Reads post.relatedBuyingGuide from the Sanity response (already fetched by updated getPostBySlug)
//   2. Renders a BuyingGuideCard between the featured products and social share, ONLY if relatedBuyingGuide is set
//   3. All existing code (featured products, hero, body, share, CTA, related articles, footer) is UNCHANGED
...
// ── Buying Guide inline card — only used in this file, no shared component modified ──
function BuyingGuideCard({ guide }) {
  if (!guide?.slug?.current) return null;
  ...
  return (
    <div
      ...
      className="pk-blog-guide-card"
    >
      {/* Hero image thumbnail — only shown if the guide has one */}
      {guide.heroImage && (
        <div
          style={{
            width: 140,
          ...
          className="pk-blog-guide-card-img"
        >
          <img
            src={urlFor(guide.heroImage).width(200).height(200).fit("crop").url()}
      ...
      <div style={{ padding: "20px 22px", flex: 1, minWidth: 0 }}>
        ...
        >
          {guide.category ? `${guide.category} · Buying Guide` : "Buying Guide"}
        </span>
  ...
  return (
    <div className="pk-no-scroll-x">
      ...
      <article className="pk-container" style={{ maxWidth: 760, marginTop: post.heroImage ? 50 : 150 }}>
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ padding: 24 }}>
                      ...
                      <h3 style={{ fontSize: 22, marginBottom: 12, lineHeight: 1.3 }}>
                        {product.title}
                      </h3>
                      <p style={{ color: "#666", lineHeight: 1.8, marginBottom: 18 }}>
                        {product.shortDescription}
                      </p>
                      ...
                      <div className="pk-btn pk-btn-gold" style={{ display: "inline-flex" }}>
                        View Product →
                      </div>
                    </div>
                  </Link>
        ...
        {/* ── NEW: RELATED BUYING GUIDE CARD ──────────────────────────────
            Only renders when post.relatedBuyingGuide is set in Sanity.
            If no guide is selected, this renders nothing — article is unchanged.
        ──────────────────────────────────────────────────────────────── */}
        {post.relatedBuyingGuide && (
          <BuyingGuideCard guide={post.relatedBuyingGuide} />
        )}
        {/* ──────────────────────────────────────────────────────────────── */}
